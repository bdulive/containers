...
rabbitmq_download_community_plugins() {
    ...
    for plugin_spec in "${plugins[@]}"; do
        ...
        # Unlike curl, wget cannot read file:// URLs, so copy those directly.
        if [[ "$plugin_url" = "file://"* ]]; then
            if ! cp "${plugin_url#file://}" "$tmp_file"; then
                rm -f "$tmp_file"
                error "Failed to read plugin from '${plugin_url}'"
                return 1
            fi
        elif ! wget --quiet --tries=3 --max-redirect 5 --output-document "$tmp_file" "$plugin_url"; then
            rm -f "$tmp_file"
